function cipher(){
    ...
    let propmt = prompt('pon una frase o palabra sin espacios');
    let cifradas = ''; 
...
//charcodeat ubica la posicion de la letra en numero
  ...
  document.write(" Tu frase cifrada es " + "<br>" + cifradas);
}

function decipher(){
    
    const cajaDecipher = document.getElementById('caja-decipher');
    let propmt = prompt('pon una palabra o frase sin espacio');
    let deCcipher = '';
  
    for(var v = 0; v < propmt.length; v++){
      
        let formu = (propmt.toUpperCase().charCodeAt(v) + 65 - 33) % 26 + 65;
    ...
    document.write(" Tu frase descifrada es " + "<br>" + deCcipher);
}
